...
from building_aware.lstm import MultiMeterSequenceDataset, fit_meter_scaler
from building_aware.metrics import horizon_metrics, regression_metrics
from building_aware.patchtst import PatchTSTLoadOnly
from building_aware.time_splits import inclusive_time_positions, split_origin_positions


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run global multi-building PatchTST baseline.")
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "global_patchtst_250.json",
...
def make_loader(
    ...
    *,
    ...
    shuffle: bool,
    num_workers: int,
) -> tuple[MultiMeterSequenceDataset, DataLoader]:
    dataset = MultiMeterSequenceDataset(
        values_norm,
...
def collect_predictions(
    ...
    *,
    ...
    meter_parts = []
    with torch.no_grad():
        for x, y, meter_idx in loader:
            x = x.to(device, non_blocking=True)
            pred = model(x).cpu().numpy()
            pred_parts.append(pred)
            true_parts.append(y.numpy())
            meter_parts.append(meter_idx.numpy())

    pred_norm = np.concatenate(pred_parts, axis=0)
    true_norm = np.concatenate(true_parts, axis=0)
    meter_indices = np.concatenate(meter_parts, axis=0).astype("int64")
    pred = scaler.inverse_transform_targets(pred_norm, meter_indices)
    true = scaler.inverse_transform_targets(true_norm, meter_indices)
    return true, pred, meter_indices
...
def save_sample_predictions(
    *,
    ...
    mask = meter_indices == sample_meter_idx
    sample_origin_positions = np.repeat(origin_positions.astype("int64"), len(meter_ids))[mask]
    target_times = time_index[sample_origin_positions + forecast_horizon]
    ...
    frame = pd.DataFrame(
        {
            ...
            "y_true": y_true[mask, horizon_idx],
            "GlobalPatchTST-load-only": y_pred[mask, horizon_idx],
        }
    )
...
def main() -> None:
    ...
    run_name = get_run_name(config, "global_patchtst")
    ...
    print(f"Task: global PatchTST load-only, past {lookback_hours} hours -> next {forecast_horizon} hours")
    ...
    print(f"train: samples={len(train_dataset)}, targets={len(train_dataset) * forecast_horizon}")
    print(f"val: samples={len(val_dataset)}, targets={len(val_dataset) * forecast_horizon}")
    print(f"test: samples={len(test_dataset)}, targets={len(test_dataset) * forecast_horizon}")
    ...
    model_name = "GlobalPatchTST-load-only"
    split_outputs = {}
    for split_name, loader in [("val", val_loader), ("test", test_loader)]:
        y_true, y_pred, meter_indices = collect_predictions(model, loader, scaler=scaler, device=device)
    ...
    for split_name, split_output in split_outputs.items():
        ...
        meter_indices = split_output["meter_indices"]
    ...
    save_sample_predictions(
        ...
        meter_indices=split_outputs["test"]["meter_indices"],
        origin_positions=origin_positions["test"],
