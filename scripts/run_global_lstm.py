...
from building_aware.lstm import LoadOnlyLSTM, MultiMeterSequenceDataset, fit_meter_scaler
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run global multi-building LSTM baseline.")
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "global_lstm_250.json",
    ...
    frame = pd.DataFrame(
        {
            ...
            "GlobalLSTM-load-only": y_pred[mask, horizon_idx],
        }
    )
...
def main() -> None:
    ...
    run_name = get_run_name(config, "global_lstm")
    ...
    print(f"Task: global LSTM load-only, past {lookback_hours} hours -> next {forecast_horizon} hours")
    ...
    model_name = "GlobalLSTM-load-only"
    split_outputs = {}
