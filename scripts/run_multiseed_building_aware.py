...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run building-aware iTransformer experiments across multiple random seeds."
    )
    parser.add_argument(
        "--base-config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "building_aware_itransformer_250_noweather.json",
        help="Base JSON config to copy and override.",
    )
    parser.add_argument(
        "--seeds",
        type=str,
        default="1,7,123,2024",
        help="Comma-separated random seeds. The base config usually keeps seed 42 as the reference run.",
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
        help="Skip a seed if its metrics file already exists.",
    )
...
def main() -> None:
    ...
    for seed in parse_seed_list(args.seeds):
        ...
        if args.skip_existing and metrics_path.exists():
            print(f"Skipping seed {seed}: {metrics_path} already exists.")
            continue
        ...
        print(f"\nRunning seed {seed} with config {config_path}...")
        subprocess.run(
            [
                sys.executable,
                str(PROJECT_ROOT / "scripts" / "run_building_aware_itransformer.py"),
                "--config",
                str(config_path),
            ],
            cwd=PROJECT_ROOT,
